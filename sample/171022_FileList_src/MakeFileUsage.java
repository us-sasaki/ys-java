...
public class MakeFileUsage {
	...
	static final String JSON_DIR = "FileChart\\json\\";
 ...
 */
	...
	private void init() throws IOException {
		// csv ファイル読込
		a = FileList.readFiles(CSV_DIR);
		a.setReferencePoint(0);
		a.ownerTable = new IoTOwnerTable();
	...
	private void makeCharts() throws IOException {
		...
		FileList.writeJsonFile(a.dateList, l1, 1, JSON_DIR+"posL1Size"+date+".json");
		List<FileEntry> l1dir = FileList.cutFile(l1, true);
		FileList.writePieChartJsonFile(l1dir, 1, JSON_DIR+"pieL1Size"+date+".json");
		...
		for (int i = 0; i < n; i++) {
			...
			FileList.writePieChartJsonFile(FileList.cut(l2big, 5), 1, JSON_DIR+"pieL2Size"+date+"_"+i+".json");
		}
		
		// レベル2で増分の大きい順に10個表示
		List<FileEntry> l2 = a.selectLevel(2);
		l2.sort(new IncreaseOrder().reversed());
		FileList.writeJsonFile(a.dateList, FileList.cut(l2, 10), 1, JSON_DIR+"lineL2Inc"+date+".json");
		
		// レベル3で増分の大きい順に10個表示
		List<FileEntry> l3 = a.selectLevel(3);
		l3.sort(new IncreaseOrder().reversed());
		FileList.writeJsonFile(a.dateList, FileList.cut(l3, 10), 2, JSON_DIR+"lineL3Inc"+date+".json");
	}
	...
	private void findSameFiles() throws IOException {
		...
		JsonArray top = JsonType.a(tableContainer);
		...
		// JSON ファイルとして出力
		//
		FileList.writeJsonType(top, JSON_DIR+"sameFile"+date+".json");
	}
	...
	private void findSimilarFiles() throws IOException {
		...
		// JSON ファイルとして出力
		//
		FileList.writeJsonType(JsonType.a(folder),
			JSON_DIR+"similarFile"+date+".json");
	}
		...
		jo.add("_values", jsonObjectArray(FileList.cut(list, 20),
			...
			(target, src) -> {
				target.add("label", src.path)
						...
						.add("owner", a.reveal(src.owner));
			}
		));
		//
		// JSON ファイルとして出力
		//
		FileList.writeJsonType(JsonType.a(jo),
			JSON_DIR+"bigFile"+date+".json");
	}
	...
	private void listFileUsage() throws IOException {
		...
		for (String name : usage.keySet() ) {
			JsonObject jo = new JsonObject().add("owner", name);
			Long u = usage.get(name);
			jo.add("usage", u.toString());
			Long lu = lastUsage.get(name);
			jo.add("lastUsage", lu.toString());
			int reductionRate = 0;
			if (lu == 0L) lu = 1L;
			if (0L != u) reductionRate = (int)((lu - u)*100L/lu);
			jo.add("reductionRate", String.valueOf(reductionRate));
			Long ds = actSize.get(name);
			jo.add("activitySize", String.valueOf(ds));
			ja.push(jo);
		}
		FileList.writeJsonType(ja, JSON_DIR+"usage"+date+".json");
		...
		for (String name : usage.keySet() ) {
			...
			JsonObject jo = JsonType.o("key", name);
			...
			jo.add("values", JsonType.a(j) );
			
			jl.push(jo);
		}
		FileList.writeJsonType(jl, JSON_DIR+"activity"+date+".json");
	}
	...
	private void createMetaData() throws IOException {
		JsonObject jo = JsonType.o("date", date);
		FileList.writeJsonType(jo, JSON_DIR+"metadata.json");
	}
