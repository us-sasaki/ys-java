...
dataset = dataset.map( (d, idx) => ({value: d, idx: idx}));
...
rect.attr("y", d => yScale(d.value))
  .attr("x", d => xScale(d.idx) + (xScale.bandwidth() - barWidth)/2)
  .attr("width", barWidth)
  ...
  .on("mouseenter", (evt, d) => {
    ...
    tooltip.style("opacity", 1)
      ...
      .style("transform", function(){ 
        let x = xScale(d.idx) + (xScale.bandwidth() / 2);
