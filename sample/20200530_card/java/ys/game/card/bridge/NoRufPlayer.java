...
/*
 * グラン・クーのディフェンダーとしてラフしない SimplePlayer を作成した。
 * しかし、ダウンさせることができるラフもしないため、没。
 */
 ...
 */
public class NoRufPlayer extends Player {
	protected Card		lead;
 ...
 * constructor
 */
	public NoRufPlayer(Board board, int seat) {
		setBoard(board);
		setMySeat(seat);
	}
	
	public NoRufPlayer(Board board, int seat, String ol) {
		this(board, seat);
		openingLead = ol;
	}
...
//			} else {
				...
				// なるべくトランプ以外を選ぶ
				int suit = board.getTrump();
				for (int i = 0; i < hand.size(); i++) {
					if (hand.peek(i).getSuit() != suit) {
						suit = hand.peek(i).getSuit();
						break;
					}
				}
