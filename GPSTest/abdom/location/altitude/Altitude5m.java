 ...
 * 前後のデータが省略されることがある。前のデータが省略される場合、データの
...
//System.out.println(f.getName() + " is found.");
				...
				// absolute path で、文字列が多い。相対にすることで
				// メモリ削減ができる。が、高が知れてるためこのまま。
				String prev = fileIndex.put(idx, f.getAbsolutePath());
				if (prev != null) {
					String prevName = new File(prev).getName();
					if (prevName.substring(18, 23).equalsIgnoreCase("dem5a") &&
							name.substring(18, 23).equalsIgnoreCase("dem5b") ) {
						System.out.println("同一メッシュで、レーザー測量版(5A)を優先します。登録：" + prev + " 破棄：" + name);
						fileIndex.put(idx, prev);
					} else {
						System.out.println("同一メッシュのレーザー測量版(5A)があったのでスキップします："+prevName);
					}
				}
			}
		}
