...
import java.io.*;
import java.util.*;
import com.ntt.util.BinaryDumpStream;
import com.ntt.util.CommentCutInputStream;

/**
 * ByteArray クラスは，byte[] に対する各種の便利なオペレーションを提供します。
 * 
 * @author		Yusuke Sasaki
 * @version		release		13, May 2002
 */
...
//	}
	...
	public static String toDumpList(byte[] data, int off, int len) {
		try {
			...
			BinaryDumpStream	bds	= new BinaryDumpStream(b);
			bds.set7Bit(true);
			while (true) {
				int c = src.read();
				if (c == -1) break;
	 ...
	 *
	 * @param		d		奇数パリティ付加対象のデータ。
	 *						最下位ビットはパリティで上書きされます
	 * @return		奇数パリティが付加された値
	 */
