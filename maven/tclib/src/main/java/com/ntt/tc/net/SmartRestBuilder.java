 ...
 * 使い方は以下の通りです。
 * <pre>
 * SmartRestBuilder srb = new SmartRestBuilder("sasatest", 100);
 * srb.setEndpoint("PUT", "/inventory/managedObject/"+srb.placeholder);
 *
 * JsonType template = new JsonObject().put("c8y_Position", new C8y_Position());
 * srb.setTemplate(template); // Jsonizable でテンプレートの元を設定
 *
 * srb.setNumberHolder("c8y_Position.lat"); // プレースホルダを指定
 * srb.setNumberHolder("c8y_Position.lng"); // プレースホルダを指定
 *
 * srb.setResponseTemplate("$","$.name","$.name");
 *
...
public class SmartRestBuilder implements Cloneable {
	...
	public static final String[] HOLDERS = { STRING, UNSIGNED, INTEGER, NUMBER, NOW, DATE};
 ...
 */
	 ...
	 * id はリクエスト用メッセージ識別子で、100, 101, のような数値を指定します。
	 * 百の位が奇数である必要があります。
	 ...
	 *						します。
	 */
	public SmartRestBuilder(String xid, int id) {
		//
		setXid(xid);
		setId(id);
	}
 ...
 */
	...
	/**
	 * テンプレート文字列を設定します。テンプレートは JSON 文字列で指定します。
	...
	public String toTemplateCsv() {
		...
		if (template == null)
			throw new IllegalArgumentException("template が設定されていません");
